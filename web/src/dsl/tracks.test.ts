...
describe("multi-fence compile", () => {
  ...
  // "track", not "synth": a `plugin` fence is a track in the same sequence, so
  // the ids they share are track ids.
  it("rejects duplicate track ids", () => {
    ...
    expect(r.errors[0]!.message).toMatch(/duplicate track id "lead"/);
  });
  ...
  it("rejects more than 8 synth fences, on the 9th", () => {
    ...
    expect(r.errors).toHaveLength(1);
    expect(r.errors[0]!.message).toMatch(/at most 8 tracks/);
...
describe("the loop as an arrangement", () => {
  ...
  it("lets phrases on different tracks use different resolutions", () => {
    ...
    expect(bassOns.map((e) => e.offsetSamples)).toEqual([
      0, 24000, 48000, 72000,
    ]);
  ...
  it("merges every line into one event list sorted by offset", () => {
    ...
    for (let i = 1; i < ev.length; i++) {
      expect(ev[i]!.offsetSamples).toBeGreaterThanOrEqual(
        ev[i - 1]!.offsetSamples,
      );
    }
  ...
  it("rejects two loop lines for the same track", () => {
    ...
    expect(r.errors[0]!.message).toMatch(
      /duplicate loop line for track "lead"/,
    );
  });
...
describe("the default document", () => {
  ...
  it("opens with canonical phrase text", () => {
    for (const phrase of Object.values(r.phrases)) {
      const fence = r.fences.find(
        (f) => f.lang === "phrase" && f.attrs["id"] === phrase.id,
      )!;
  ...
  it("keeps the lead chords, strummed by the roll gesture", () => {
    ...
    expect(first.map((e) => e.offsetSamples)).toEqual([
      0,
      Math.round(0.009 * SR),
      Math.round(0.018 * SR),
    ]);
  ...
  it("accents the hat where the detail block says so", () => {
    ...
    expect(hats.filter((e) => Math.abs(e.velocity - 0.85) < 1e-6)).toHaveLength(
      2,
    ); // beat 2
    expect(hats.filter((e) => Math.abs(e.velocity - 0.55) < 1e-6)).toHaveLength(
      6,
    );
  });
  ...
  it("drives the kick pitch from the filter envelope", () => {
    ...
    expect(kick.ir.mod).toEqual([
      { from: "env.filter", to: "pitch", amount: 3600 },
    ]);
  });

  it("only puts fx on the lead", () => {
    expect(
      r.tracks.find((t) => t.id === "lead")!.ir.fx.map((f) => f.type),
    ).toEqual(["reverb", "comp"]);
